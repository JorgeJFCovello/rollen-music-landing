...
- The cat motif is one SVG silhouette (`CatSilhouette`, single `currentColor` fill, no gradient, no stroke detail) reused at three sizes: the Hero "Listen" mark, small icons inside the Follow niches, and one quiet detail sitting inside a Bio tablet's top-right corner.
...
**The Contained Decoration Rule.** Nothing painted "onto" a Stone Panel (coffee stains, the perched Cat Silhouette) may visually hang off its edge onto the dark ground behind it — a stain or mark is a mark *on the page*, not a sticker floating past its corner. Every Bio tablet carries `overflow-hidden` specifically so absolutely-positioned decoration gets cropped to the panel's own rectangle; when placing a new decorative element on a panel, position it to sit within (or crop cleanly at) that boundary rather than relying on it bleeding attractively onto the section background.
...
### Coffee Stain
`CoffeeStain` in `ornaments.tsx` — a mottled, organic watercolor blot: three layered hand-authored blob paths (light outer wash → mid wash → dark off-center core), thin drip tendrils off one edge, and small scattered splatter dots outside the main blot. The whole blot passes through a `feTurbulence`+`feDisplacementMap` filter for a bled, non-geometric edge, plus a second `feTurbulence`+`feColorMatrix`+`feComposite` filter (composited `in` the shape's own alpha, so the grain never spills past its boundary) for internal mottling. Takes a required `id` prop that seeds its local filter defs (each instance needs unique filter IDs in the same document), and a `variant` prop (`"a"` default, `"b"`, `"c"`) selecting one of three genuinely distinct geometries — different outer silhouette, different core position, different drip direction, different splatter placement — never just a resize/rotate of the same shape. This exists because the first two-tablet pass reused variant `a` at a different size/rotation on both, and the user pointed out it read as "the exact same stain" copy-pasted. Three instances now live on Bio's two tablets: `variant="a"` (`h-32/h-36`, `rotate-[8deg]`, bottom-right) on the first tablet; `variant="b"` (elongated, core lower-right, side drips; `h-28/h-32`, `-rotate-[12deg]`, top-left, nudged up to `-top-12` so it clears the body text) and `variant="c"` (a double-lobed "set down twice" blot, wide and low; `h-24/h-28`, `rotate-[4deg]`, anchored `right-2/4 top-1/2 -translate-y-1/2`) both on the second tablet — `c` was placed deliberately in the gap between that tablet's two paragraphs, on the right, rather than overlapping either paragraph's text. Not a ring/donut shape — that was the very first attempt and was rejected as looking "pretty bad" against a real coffee-stain reference photo. Any fourth instance added later must get its own new variant, not a reuse of `a`, `b`, or `c` at a different transform.
